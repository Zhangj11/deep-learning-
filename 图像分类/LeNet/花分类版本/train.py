...
data_transform = {
    "train":transforms.Compose([
            transforms.Resize((28,28)),#调整图像的尺寸为28*28
    ...
    "val":transforms.Compose([
          transforms.Resize((28,28)),#裁剪为224*224
          transforms.ToTensor(),#将0-255范围内的像素转为0-1范围内的tensor
          transforms.Normalize(std=[0.5,0.5,0.5],mean=[0.5,0.5,0.5])#归一化
])}
...
#定义优化器adam,随机梯度下降，原来是sgd
optimizer = torch.optim.Adam(model.parameters(), lr=0.0003)#跟googlenet一样
...
#定义验证函数
def val(dataloader,model,loss_fn,i,epoch):
    # 将模型转为验证模式
    model.eval()
...
for i in range(epoch):
    ...
    train_loss,train_acc=train(train_dataloader, model, loss_fn, optimizer,i,epoch)#调用train函数
